...
include_once(dirname(realpath(dirname(__FILE__))).'/function/config.inc.php');
	...
	$aColumns = array('id','name','slug', 'icon','position','menutop','menufoot');
	...
	$sWhere .= "`status` IN ('1') AND `iscategory` != 0";
	...
	//$aRow = mysql_fetch_array( $rResult ) ;
		$img = "";
		$menutop = '<i class="fa fa fa-close"></i>';
		$menufoot = '<i class="fa fa fa-close"></i>';
		if($aRow['icon'])
			$img = "<img src='".$aRow['icon']."' >";
		if($aRow['menutop'])
			$menutop = '<i class="fa fa fa-check"></i>';
		if($aRow['menufoot'])
			$menufoot = '<i class="fa fa fa-check"></i>';
		$row = array();
		$row[] = $i;  
		$row[] = $aRow['name'];       		
		$row[] = $aRow['slug'];
		...
		$row[] =' <div class="btn-group"> <a class="btn btn-mini btn-success dropdown-toggle btn-demo-space" data-toggle="dropdown" href="#">Action <span class="caret"></span> </a>
					<ul class="dropdown-menu">
					  <li><a href="category.php?id='.$aRow['id'].'">Edit</a></li>
					  <li class="divider"></li>
					  <li><a href="php/category.php?action=del&id='.$aRow['id'].'" >Delete</a></li>
					</ul>
				  </div>';
